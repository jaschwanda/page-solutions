...
class USI_Page_Dbs extends mysqli {
   ...
   const VERSION = '1.5.3 (2020-02-20) [USI_Dbs:1.1.3 (2018-01-18)]';
   ...
   function __construct($options) {
      ...
      if ($this->connect_errno) throw new USI_Page_Dbs_Exception(__METHOD__. ':connect:' . $this->connect_error);
   } // __construct();

   public function prepare_x($statement, $inputs = null, $outputs = null, $execute = true, $store = true) {
      $query = new USI_Page_Dbs_Stmt($this, $statement);
      $status = $query->prepare($statement);
      if (!$status) throw new USI_Page_Dbs_Exception(__METHOD__. ':' . str_replace(USI_Page_Dbs::PREPARE_ERROR_1, '', $this->error));
      if ($inputs) {
...
class USI_Page_Dbs_Exception extends Exception { } // Class USI_Page_Dbs_Exception;
...
class USI_Page_Dbs_Stmt extends mysqli_stmt {
   ...
   public function bind_param_x($args) {
      ...
      if (!$status) {
         ...
         throw new USI_Page_Dbs_Exception(__METHOD__ . ':' . self::$errstr);
      }
      return($status);
   } // bind_param_x();
   ...
   public function fetch_x() {
      $status = parent::fetch();
      if (false === $status) throw new USI_Page_Dbs_Exception(__METHOD__ . ':' . $this->error);
      return($status);
   } // fetch_x();
